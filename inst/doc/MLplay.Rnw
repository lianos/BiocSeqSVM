...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item The \href{}{vignette from the caret package} by Max Kuhn.
  \end{itemize}
  
  \item We will be exploring support vector machines through a new R library
  I am authoring together called \href{https://gihub.com/lianos/shikken}{shikken}.
  While there are several R libraries that provide an interface to SVMs, shikken
  is a wrapper to the
  \href{http://www.shogun-toolbox.org/}{Shogun machine learning toolbox}.
  Shogun is a \texttt{C++} library focussed on large scale kernel methods and
...
An advantageous property of SVMs, as shown in Figure~\ref{fig:svmdecision} is that
they build large margin linear classifiers that, typically, live in high
dimensional spaces. The margin of a classifier is simply defined as the shortest
distance from the decision boundary to the first training example. Large margin
classifiers have the tendency to generalize well to unseen data --- intuitively
because they provide more ``breathing room'' between the decision boundary and
the next unseen data point you want to classify.
