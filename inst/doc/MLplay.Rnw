...
A reasonable way to split the data in Figure~\ref{fig:circledata} is to
project it into a higher dimensional space where it can then be
cleanly separated by a simple line (hyperplane) in that new space.
At first blush, a naive way to do this is to explicitly project
every data point into the new space and just find the hyperplane
like ``you normally would.'' Your ${\bf w}$ vector would then 
also be living in this space and your new discriminant function would
then look like what is shown in Equation~\ref{eqn:phidisc}.
...
Our discriminant function is therefore:

\begin{align}
  f({\bf x}) = \sum_{i=1}^n y_i \alpha_i k({\bf x_i}, {\bf x}) + b
\end{align}
...
Now, we're trying to drive to the point that we can use a linear classifier to
classify non-separable data by projecting the data into a higher dimensional
space. We can naively do this by 
...
as shown in Figures~\ref{fig:polykernel}, and ~\ref{fig:gausskernel}.

Note that in the dual, the SVM optimization depends on the inputs ${\bf x}_i$
only through their dot products, which allows the algorithm to be
\emph{kernelized}.

\paragraph{Important} An advantage of measuring relationships between examples
through kernel functions is that we often do not have to
