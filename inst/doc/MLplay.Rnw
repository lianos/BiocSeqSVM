...
shed light on things that may at first seem to be too abstract.
...
Equation~\ref{eqn:softmargin}, and depicted in Figure~\ref{fig:svmdecision}.
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    identified by the blue line. This figure was adapted from the Wikipedia page about
    \href{http://en.wikipedia.org/wiki/Support_vector_machine}{Support vector machines}
    on 2012-02-22.
    }
  }
...
\paragraph{Question} \emph{What happens to the decision boundary and support vectors
...
\paragraph{Question} \emph{What should we do instead?
Is the best classifier you can build the
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    SVM ``enough slack'' to misclassify the ``outlier''.
  }
...
\begin{figure}[htbp]
  \centering
    \includegraphics[scale=.2]{Rfigs/gen-circleData}
  \caption{Data that is not linearly separable.}
  \label{fig:circledata}
\end{figure}

There is a \emph{dual} formulation of the SVM objective function that uses
Lagrange multipliers and its optimal value is the same as the primal
one under certain constraints. The derivation of the dual from the primal
is skipped here (for more details, the reader can reference one of many
SVM tutorials). The final equation is listed in Equation~\ref{eqn:dual}. Note
that in the dual, the SVM optimization depends on the inputs ${\bf x})i$
only through their dot products, which allows the algorithm to be
\emph{kernelized}:

\begin{align}
  \max_a \sum_{i=1}^n \alpha_i - \frac {1} {2} \sum_{i=1}^n \sum_{j=1}^n y_i y_j \alpha_i \alpha_j \left\langle {\bf x}_i, {\bf x}_j \right\rangle \nonumber  \\
  \mbox{s.t : } \sum_{i=1}^n y_i \alpha_i = 0; \mbox{ } 0 \leq a_o \leq C
\label{dual}
\end{align}

The weight vector ${\bf w}$ can be expressed by the examples ${\bf x}_i$ and
their weights $\alpha_i$ as shown in Figure~\ref{fig:wvector}. The examples which have $\alpha_i > 0$ are called the \emph{support vectors}, which lie on (or within)
the margin (these are the red examples shown in the \texttt{plotDecisionSurface})
function.
...
\paragraph{Important} A, perhaps non-obvious, advantage of measuring relationships
between examples through kernel functions is that we often do not have to
explicitly map examples into the higher dimensional feature space to calculate their similarity. Their projection can be implicit, which can provide large memory and 
computational cost gains. Refer to the polynomial kernel example below.
...
To extract the ${\bf w}$ weight vector:
...
\begin{align}
  k_{d,k}^{\mbox{polynomial }} ({\bf w}, {\bf w'}) = \left( \left\langle {\bf w}, {\bf w'} \right\rangle + k \right)^d
...
\begin{align}
  k_{d,k}^{\mbox{gaussian }} ({\bf w}, {\bf w'}) = \exp \left( - \frac {1} {\sigma} \| {\bf w}, {\bf w'} \|^2 \right)
\end{align}
...
$u$ in the string $x$. An intuitive representation of the kernel kernel is shown
in Figure~\ref{fig:spectrum}
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    A naive way to calculate the spectrum kernel of degree 2 is shown.
    A sliding window (red box) equal to length as the degree of the kernel
    slides along the string. The exact $k$-mers are counted as they appear
    (above). We can allow for a mismatch by incrementing the count of all
    of the kmers in an $N$-mismatch neighborhood around the current $k$-mer
    in the window (below). The kernel function to compare two strings
    is simply an inner product of their vector representations.
  }
  \label{fig:spectrum}
\end{figure}
...
head(promoters)
# X.prom <- spectrumFeatures(promoters, degree=4)
# y.prom <- values(promoters)$class

# ssvm <- SVM(X.prom, y.prom, kernel='linear')
# table(predict(ssvm, X.proms), pr.class)
@
