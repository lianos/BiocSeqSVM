...
lsvm <- showSVM(X, y, kernel="linear", C=10, scale=1)
gsvm <- showSVM(X, y, kernel="gaussian", C=10, width=1)

## What happens when it's not linearly separable?
X1 <- rbind(X1, matrix(c(-1, 0), ncol=2))
X <- rbind(X1, X2)
y <- c(rep(1, N+1), rep(-1, N))
@
...
##
X <- matrix(c(rnorm(2 * N, 1, 1), rnorm(2 * N, -1, 1)), ncol=2)

xlim <- c(min(X[,1]) -1, max(X[,1]) + 1)
ylim <- c(min(X[,2]) -1, max(X[,2]) + 1)

matplot(X[1:N, 1], X[1:N, 2], pch="+", col="blue", cex=1.5, xlim=xlim, ylim=ylim)
matplot(X[-(1:N), 1], X[-(1:N), 2], pch="-", col="red", cex=1.5, add=TRUE)


lsvm <- showSVM(X, y, kernel="linear", C=10)
cat(sprintf("Linear svm accuracy: %.2f\n", sum(predict(lsvm, X) == y) / length(y)))


gsvm <- showSVM(X, y, kernel="gaussian", C=1000, width=1)
