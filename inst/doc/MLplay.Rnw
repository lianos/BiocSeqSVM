...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    This decision boundary is has the maximum margin in that it is equally distant
    from the closest positive (filled) and negative (hollow) examples.
    The examples with dark borders are the support vectors, which orient the 
    direction of $\vec{W}$. The size of the margin for this classifier is identified
    by the blue line. The red support vector is allowed to cross the margin,
    and does not shift the decision boundary by paying a penalty that is proportional
    to $\xi_i$. (Figure adapted form wikipedia)
  }
  \label{fig:svmdecision}
\end{figure}

The margin of a linear classifier is defined as the distance of the closest point
to the decision boundary --- SVMs build classifiers that maximize this distance.

The idea of the maximum margin is shown in~Figure\ref{fig:svmdecision}.

Details of the optimization problem are summarized from~\cite{BenHur:2008ec}

The hard margin SVM optimizes the following function:
...
The constraints in equation~\ref{eqn:primal} ensure that the examples are
correctly classified and minimizing $\| {\bf w} \|^2$ is equivalent to
maximizing the margin. In practice, data are often not cleanly separable.
The idea of a soft margin was introduce to accommodate these problems. The
soft margin allows the classifier to misclassify some examples in order keep
the margin large. Soft margin classifiers typically provide better performance
than hard margin SVMs. To allow for errors, \emph{slack variables} ($\xi_i$)
are introduced into the inequality constraint in Equation~\ref{eqn:primal}
to permit examples to lie within the margin or to be misclassified. To
discourage excess use of slack variables, a cost parameter $C$ is also
introduced. The soft margin optimization function follows:
...
Kernels project data into different spaces. The SVM can employ a kernel
expansion
...
lsvm <- SVM(X, y, kernel="linear", C=10)
table(predict(lsvm, X), y)

plotDecisionSurface(lsvm, X, y)
@

And what happens when data moves closer to the margin?

<<label=easyMarginMove,fig=TRUE,include=FALSE>>=
X.harder <- rbind(X, matrix(c(0, 0), ncol=2))
y.harder <- c(y, 1)

lsvm <- SVM(X.harder, y.harder, kernel="linear", C=10)
table(predict(lsvm, X.harder), y.harder)

plotDecisionSurface(lsvm, X.harder, y.harder)
@

\begin{figure}[htbp]
  \centering  
  \mbox{\subfigure{\includegraphics[width=3in]{MLplay-easyMargin.pdf}}\quad
  \subfigure{\includegraphics[width=3in]{MLplay-easyMarginMove.pdf} }}
  \caption{Finding the maximum margin hyperplane in shifting data}
...
\begin{figure}[htbp]
  \centering  
  \mbox{\subfigure{\includegraphics[width=3in]{MLplay-svmPoly.pdf}}\quad
  \subfigure{\includegraphics[width=3in]{MLplay-svmPoly3D.pdf} }}
...
\begin{figure}[htbp]
  \centering  
  \mbox{\subfigure{\includegraphics[width=3in]{MLplay-svmGaus.pdf}}\quad
  \subfigure{\includegraphics[width=3in]{MLplay-svmGaus3D.pdf} }}
