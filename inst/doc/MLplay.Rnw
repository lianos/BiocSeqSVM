...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    identified by the blue line.\footnote{
      This figure was adapted from wikipedia:
      \url{http://en.wikipedia.org/wiki/Support_vector_machine} [2012-02-22]
    }
  }
...
As we saw in the previous example, not all data is well behaved --- meaning
that it is not easy to find a linear separator that can split your two classes
...
<<label=circleData,fig=TRUE,include=TRUE>>=
...
\begin{figure}[htbp]
  \centering
    \includegraphics[scale=1]{figs/spcectrum}
  \caption{
...
Randomize the labels and try again -- can the SVM learn a perfect classifier?
<<label=randomPromoter>>=
y.rand <- sample(y.prom)
rsvm <- SVM(promoters, y.rand, kernel='spectrum', degree=4)
table(predict(rsvm, promoters), y.prom)

rsvm <- SVM(promoters, y.rand, kernel='spectrum', degree=8)
table(predict(rsvm, promoters), y.prom)
@
